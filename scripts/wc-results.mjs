/*
 * scripts/wc-results.mjs — Lógica compartida de resultados del Mundial 2026
 * =========================================================================
 * Mapeo de equipos, fixture compacto y obtención de partidos FINALIZADOS desde
 * football-data.org. Lo usan tanto el sync de las ligas fijas
 * (sync-results.mjs) como el del hub multi-liga (sync-results-hub.mjs).
 */
...
export function findMatchId(homeKey, awayKey) {
  const m = MATCHES.find((x) => x.home === homeKey && x.away === awayKey);
  return m ? m.id : null;
}

export function resolveTeam(name, shortName, tla) {
  return TEAM_MAP[name] || TEAM_MAP[shortName] || TEAM_MAP[tla] || null;
}

/*
 * Obtiene los partidos FINALIZADOS del Mundial 2026 y los traduce a nuestros
 * IDs internos. Devuelve { results: [{match_id, home_goals, away_goals}], skipped }.
 */
export async function fetchFinishedResults(footballKey) {
  ...
  if (!apiRes.ok) {
    ...
    throw new Error(`football-data.org respondió ${apiRes.status}: ${text}`);
  }
  ...
  for (const match of finishedMatches) {
    ...
    results.push({ match_id: matchId, home_goals: homeGoals, away_goals: awayGoals });
  }
